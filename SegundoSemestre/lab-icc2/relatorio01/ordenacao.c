...
void rodandoNums(int tam, int v[], FILE *csv, int op){
...
int main(){
    srand(time(NULL));
    FILE *csv = fopen("ordenacao2.txt", "w");
    fprintf(csv, "[tam   ], [tempo   ]\n");
    ...
    for (int j = 0; j < 3; ++j) {
        ...
        for (int i = 0; i < 4; ++i) {
            if (i == 0)
                rodandoNums(tam25, v25, csv, j);
            else if (i == 1)
                rodandoNums(tam100, v100, csv, j);
            else if (i == 2)
                rodandoNums(tam1000, v1000, csv, j);
            else{
                for (int k = 2; k <= 10; ++k) {
                    int *vk = malloc(k*1000*sizeof(int));
                    rodandoNums(k*1000, vk, csv, j);
                    free(vk);
                }
